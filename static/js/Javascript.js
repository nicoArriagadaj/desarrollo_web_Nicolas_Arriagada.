...
// falta que sea imagen, no un txt
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const comunasPorRegion = {
    arica: obtenerComuna("Región Arica y Parinacota"),
    tarapaca: obtenerComuna("Región de Tarapacá"),
    antofagasta: obtenerComuna("Región de Antofagasta"),
    atacama: obtenerComuna("Región de Atacama"),
    coquimbo: obtenerComuna("Región de Coquimbo "), 
    valparaiso: obtenerComuna("Región de Valparaíso"),
    metropolitana: obtenerComuna("Región Metropolitana de Santiago "), 
    ohiggins: obtenerComuna("Región del Libertador Bernardo Ohiggins"),
    maule: obtenerComuna("Región del Maule"),
    nuble: obtenerComuna("Región del Ñuble"),
    biobio: obtenerComuna("Región del Biobío"),
    araucania: obtenerComuna("Región de La Araucanía"),
    losrios: obtenerComuna("Región de Los Ríos"),
    loslagos: obtenerComuna("Región de Los Lagos"),
    aysen: obtenerComuna("Región Aisén del General Carlos Ibáñez del Campo"),
    magallanes: obtenerComuna("Región de Magallanes y la Antártica Chilena")
  };
...
function correoValido() {
  ...
  if (CorreoIngresado.length === 0) {
    ...
    errorGeneral.textContent = "Debe ingresar un correo electrónico válido.";
    return false;
  }
  ...
  if (CorreoIngresado.length > 100) {
    ...
    CorreoError.style.color = "red";
    errorGeneral.textContent = "El correo excede los 100 caracteres.";
    return false;
  }
  ...
  if (!regexEmail.test(CorreoIngresado)) {
    ...
    CorreoError.style.color = "red";
    errorGeneral.textContent = "Formato de correo inválido.";
    return false;
  }
  ...
  for (let input of fotos) {
    if (input.files.length > 0) {
      ...
      if (!archivo.type.startsWith('image/')) {
        errorGeneral.textContent = "Solo se permiten archivos de imagen (JPG, PNG, etc.).";
        return false;
      }
    }
  }
